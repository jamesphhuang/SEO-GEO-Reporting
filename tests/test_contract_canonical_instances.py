...
class CanonicalContractInstanceTests(unittest.TestCase):
    def test_registry_has_exactly_seven_authoritative_logical_ids(self):
    ...
    def test_registry_maps_logical_ids_to_proposal_files_and_consumers(self):
    ...
    def test_unknown_logical_id_fails_closed(self):
        with self.assertRaisesRegex(CanonicalInstanceError, expected_regex="unknown logical contract id"):
            get_contract_spec("unknown.v1")


    def test_registry_rejects_duplicate_or_ambiguous_mappings(self):
        ...
        with self.assertRaisesRegex(CanonicalInstanceError, expected_regex="duplicate logical contract id"):
            _validate_registry((spec, duplicate))


    def test_environment_dimensions_are_independent_and_fail_closed(self):
    ...
    def test_legacy_environment_requires_explicit_migration(self):
        with self.assertRaisesRegex(CanonicalInstanceError, expected_regex="requires explicit"):
    ...
    def test_all_seven_instances_are_materialized(self):
    ...
    def test_fixture_contains_the_same_seven_instances(self):
    ...
    def test_materialization_is_deterministic_across_repeated_runs(self):
    ...
    def test_semantic_field_order_does_not_change_hash(self):
    ...
    def test_contract_revision_is_semantic_and_changes_hash(self):
    ...
    def test_runtime_trace_metadata_does_not_change_semantic_hash(self):
    ...
    def test_runtime_event_revision_cannot_replace_contract_revision(self):
        with self.assertRaisesRegex(CanonicalInstanceError, expected_regex="unknown semantic field"):
            materialize_contract_instance("human_review.v1", semantic_overrides={"revision": 9})


    def test_unknown_semantic_override_fails_closed(self):
        with self.assertRaisesRegex(CanonicalInstanceError, expected_regex="unknown semantic field"):
            materialize_contract_instance("human_review.v1", semantic_overrides={"created_at": "now"})


    def test_runtime_and_external_metadata_cannot_become_semantic_fields(self):
        with self.assertRaisesRegex(CanonicalInstanceError, expected_regex="unknown semantic field"):
            materialize_contract_instance(
                "human_review.v1",
    ...
    def test_identity_binding_and_workbook_ids_are_external_to_contract_instances(self):
    ...
    def test_canonical_instances_never_authorize_production(self):
    ...
    def test_production_activation_instance_is_governance_only(self):
    ...
    def test_instances_contain_no_external_identity_or_target_values(self):
